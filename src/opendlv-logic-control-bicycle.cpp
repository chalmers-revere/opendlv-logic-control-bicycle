...
int32_t main(int32_t argc, char **argv)
{
  ...
  if ( (0 == commandlineArguments.count("cid")) ||
    ...
    std::cerr << argv[0] << " controls a vehicle by using an inverted bicycle "
      ...
      << "--speed-max=<Maximum speed> --cog-to-front=<Distance COG to front> "
      ...
      << std::endl
      << "Example: " << argv[0] 
      << " --cid=111 --cog-to-left=1.2 --cog-to-rear=1.3" << std::endl;
  } else {
    ...
    auto onGroundDecelerationRequest{[&speedControl, &speedControlMutex,
    ...
    auto onPedalPositionRequest{[&speedControl, &speedControlMutex,
      &verbose](cluon::data::Envelope &&envelope)
      {
        auto msg = 
    ...
    auto atFrequency{[&od4, &vehicleLength, &cogToRear, &speedMax, &vxRequest, 
      &yawRateRequest, &senderStampOutput, &requestMutex, &verbose]() -> bool
      {
        float delta;
        {
          std::lock_guard<std::mutex> lock(requestMutex);
        ...
        if (verbose) {
          std::cout << "Sending steering wheel angle=" << delta << std::endl;
        }
